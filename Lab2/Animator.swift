...
class Animator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using context: UIViewControllerContextTransitioning) {
        // push
        if let discoverVC = context.viewController(forKey: UITransitionContextViewControllerKey.from) as? ViewController,
            let detailVC = context.viewController(forKey: UITransitionContextViewControllerKey.to) as? SongController {
            moveFromCategories(discoverVC: discoverVC, toList: detailVC, withContext: context)
        }
            
            // pop
        else if let discoverVC = context.viewController(forKey: UITransitionContextViewControllerKey.to) as? ViewController,
            let detailVC = context.viewController(forKey: UITransitionContextViewControllerKey.from) as? SongController {
            moveFromList(detailVC: detailVC, toDiscover: discoverVC, withContext: context)
        }
    }
    
    private func moveFromCategories(discoverVC: ViewController, toList detailVC: SongController, withContext context: UIViewControllerContextTransitioning) {
    ...
    private func moveFromList(detailVC: SongController, toDiscover discoverVC: ViewController, withContext context: UIViewControllerContextTransitioning) {
